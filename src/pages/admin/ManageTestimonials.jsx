...
const ManageTestimonials = () => {
  ...
  const openEdit = (t) => {
    setEditId(t._id);
    setForm({ name: t.name, role: t.role || '', content: t.content, rating: t.rating || 5, isActive: t.isActive ?? true });
    setModalOpen(true);
  };
              ...
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center">
                    <span className="font-body font-medium text-accent text-sm">{t.name?.charAt(0)}</span>
                  </div>
                  <div>
                    <p className="font-body text-sm font-medium text-text">{t.name}</p>
                    {t.role && <p className="font-body text-xs text-text/40">{t.role}</p>}
                  </div>
                </div>
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-primary font-body font-medium text-sm mb-1.5">Role (optional)</label>
                  <input
                    type="text"
                    value={form.role}
